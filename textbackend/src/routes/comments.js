...
router.delete('/:id', requireAuth, perUserRateLimit({ max: 60 }), requireRole('Admin'), validate(idParam), commentsController.remove);

// Approve a comment - Admin only
router.post('/:id/approve', requireAuth, perUserRateLimit({ max: 30 }), requireRole('Admin'), validate(idParam), commentsController.approve);

// Reject a comment (soft-delete) - Admin only
router.post('/:id/reject', requireAuth, perUserRateLimit({ max: 30 }), requireRole('Admin'), validate(idParam), commentsController.reject);
